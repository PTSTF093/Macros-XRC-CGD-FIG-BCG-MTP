...
#
#
#		Macro monitorização XRC CG01 da CGD
#
#
#
#
...
function prelimpeza() 
{
	...
	ps -ef | grep 'title CGD' | awk '{print $2}' | xargs kill
...
function matanca() 
{
	#ps -ef | grep 'zenity' | awk '{print $2}' | xargs kill
	ps -ef | grep 'title CGD' | awk '{print $2}' | xargs kill
	#ps -ef | grep 'CGD_XRC_macro.sh' | awk '{print $2}' | xargs kill
	for pid in $(ps -ef | awk '/CGD_XRC_macro.sh/ {print $2}'); do kill -9 $pid; done
...
do
	...
	if [[ "$hm" -lt "$now_m1" ]] || [[ "$hm" -gt "$now_M1" ]]; then
		...
		BEEP
		zenity --info --text "CGD - Hora do emulador não está a atualizar $now != $hora_emulador" &
		#exit 8
	fi
